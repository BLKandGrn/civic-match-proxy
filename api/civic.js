export default async function handler(req, res) {
  ...
  try {
      ...
      if (endpoint === "elections") {
              ...
              const defaultAddress = "510 Cranston Ave, Upper Marlboro, MD 20774";
              const lookupAddress = address || defaultAddress;
              const url = `https://www.googleapis.com/civicinfo/v2/voterinfo?key=${GOOGLE_KEY}&address=${encodeURIComponent(lookupAddress)}&returnAllAvailableData=true`;
              const response = await fetch(url);
              const data = await response.json();
